...
class BME680(BME680Data):
    ...
    def __init__(self, i2c_addr=constants.I2C_ADDR_PRIMARY, i2c_device=None):
        ...
        try:
            ...
            raise RuntimeError(
                "Unable to identify BME680 at 0x{:02x} (IOError)".format(self.i2c_addr)
            )
    ...
    def _get_calibration_data(self):
        ...
        heat_value = constants.twos_comp(
            self._get_regs(constants.ADDR_RES_HEAT_VAL_ADDR, 1), bits=8
        )
        sw_error = constants.twos_comp(
            self._get_regs(constants.ADDR_RANGE_SW_ERR_ADDR, 1), bits=8
        )
    ...
    def set_humidity_oversample(self, value):
        ...
        self._set_bits(
            constants.CONF_OS_H_ADDR, constants.OSH_MSK, constants.OSH_POS, value
        )

    def get_humidity_oversample(self):
        """Get humidity oversampling."""
        return (
            self._get_regs(constants.CONF_OS_H_ADDR, 1) & constants.OSH_MSK
        ) >> constants.OSH_POS
    ...
    def set_pressure_oversample(self, value):
        ...
        self._set_bits(
            constants.CONF_T_P_MODE_ADDR, constants.OSP_MSK, constants.OSP_POS, value
        )

    def get_pressure_oversample(self):
        """Get pressure oversampling."""
        return (
            self._get_regs(constants.CONF_T_P_MODE_ADDR, 1) & constants.OSP_MSK
        ) >> constants.OSP_POS
    ...
    def set_temperature_oversample(self, value):
        ...
        self._set_bits(
            constants.CONF_T_P_MODE_ADDR, constants.OST_MSK, constants.OST_POS, value
        )

    def get_temperature_oversample(self):
        """Get temperature oversampling."""
        return (
            self._get_regs(constants.CONF_T_P_MODE_ADDR, 1) & constants.OST_MSK
        ) >> constants.OST_POS
    ...
    def get_filter(self):
        """Get filter size."""
        return (
            self._get_regs(constants.CONF_ODR_FILT_ADDR, 1) & constants.FILTER_MSK
        ) >> constants.FILTER_POS
        ...
        if value > constants.NBCONV_MAX or value < constants.NBCONV_MIN:
            raise ValueError(
                "Profile '{}' should be between {} and {}".format(
                    value, constants.NBCONV_MIN, constants.NBCONV_MAX
                )
            )
    ...
    def get_gas_heater_profile(self):
        ...
        return (
            self._get_regs(constants.CONF_ODR_RUN_GAS_NBC_ADDR, 1)
            & constants.NBCONV_MSK
        )
    ...
    def get_gas_heater_status(self):
        ...
        return (
            self._get_regs(constants.CONF_HEAT_CTRL_ADDR, 1) & constants.HCTRL_MSK
        ) >> constants.HCTRL_POS
    ...
    def set_power_mode(self, value, blocking=True):
        ...
        self._set_bits(
            constants.CONF_T_P_MODE_ADDR, constants.MODE_MSK, constants.MODE_POS, value
        )
    ...
    def get_sensor_data(self):
        ...
        for attempt in range(10):
            ...
            if self._variant == constants.VARIANT_HIGH:
                self.data.gas_resistance = self._calc_gas_resistance_high(
                    adc_gas_res_high, gas_range_h
                )
            else:
                self.data.gas_resistance = self._calc_gas_resistance_low(
